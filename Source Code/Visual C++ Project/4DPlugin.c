...
struct		WINDOWHANDLES
{
	...
	HWND		prtSettingshWnd; // handle yo print settings dialog
...
// --- Win32API Commands
		...
		case kInitPlugin :
		  ...
		  //hWnd = (HWND)Blk4D.fHandle;
			...
			windowHandles.MDIs_4DhWnd = (HWND)PA_GetHWND(0); // this should be same as above
...
{
	PA_Variable				printerArray, localPrinters, remotePrinters;
	PA_Variable				*ptrPA_Var;
	long							returnValue = 0, loopOffset, action = 0, defPrinterPosition;
	DWORD							dwSizeNeeded, cByteNeeded, cByteUsed;
	DWORD							dwNumItems, dwLocalNumItems, dwTotalNumPrinters, dwRemoteNumItems = 0;
	DWORD							dwItem;
	...
	BOOL							bPrinterSuccess, bRemoteOnly, bNamesOnly = FALSE;
	...
	switch (action)
	{
		...
		case EP_USE_REGISTRY :
			returnValue = sys_GetOSVersion(TRUE, params);
			switch (returnValue)
			{
				case OS_WIN95 :
				case OS_WIN98	:
				case OS_ME		:
					defPrinterPosition = enumPrintersUsingINI( &printerArray );
					break;
				default :
	...
	EnumPrinters ( PRINTER_ENUM_LOCAL, NULL, 1, NULL, 0, &dwSizeNeeded, &dwNumItems );
	...
	for ( dwItem = 0; dwItem < dwNumItems; dwItem++ )
	{
		...
		PA_SetStringInArray (localPrinters, dwItem+1, printerName);
	}
	// free memory
	HeapFree ( GetProcessHeap (), 0, lpInfo1 );
  ...
  //sprintf(debugStr,"Number of elements: %d",PA_GetArrayNbElements (printerArray));
	...
	{
		PA_GetStringInArray( *ptrPA_Var, loopOffset + dwItem + 1, printerName);
		printerName[strlen(printerName)] = '\0';
		strcpy(printer_driver_port, "");

		bPrinterSuccess = OpenPrinter( printerName, &hPrinter, NULL ); //&ptrDef );

		if ((bPrinterSuccess) && (!bNamesOnly)) {		
...
void sys_GetPrintJob( PA_PluginParameters params)
{
	...
	if (strlen(printerSettings.printerSelection) == 0) {  
		...
		PA_SetTextInArray (printer, 7, returnString,
			strlen(returnString));
...
void gui_GetWindow( PA_PluginParameters params, HWND hWnd )
{
  ...
  windowTitle[windowTitle_len] = '\0';  // Explicitly set the length
	...
	if (strcmp(windowTitle, "*") == 0) { // return the frontmost window
		...
		if ( (strlen(windowTitle) == 0) && (windowHandles.fourDhWnd != NULL) ) {
	...
	if(IsWindow(WindowhWnd)) {
		...
		*/
		...
		if(GetMonitorInfo(hMon4D, &monitorInfo)) {
			if (GetWindowRect(WindowhWnd, &wRect)) {
				/*
			  x = monitorInfo.rcMonitor.left + wRect.left;
			  y = monitorInfo.rcMonitor.top + wRect.top;
			  w = (monitorInfo.rcMonitor.left + wRect.right) - x;
		  	h = (monitorInfo.rcMonitor.top + wRect.bottom) - y;
				*/
...
void sys_GetTimeZone( PA_PluginParameters params )
{
	...
	HKEY		hKey, rootKey;
	DWORD		dwType = REG_SZ;
	DWORD		dwDaylightType = REG_SZ;
	DWORD		dwAutoDaylight = 0;
	DWORD		dwSize = 255;
	DWORD		dwDaylightSize = 255;
	char		sz[255];
	char		szDaylight[255];
	DWORD		resultSize = 255;
	char		autoDaylightSubkeyName[30] = "DisableAutoDaylightTime";
	BOOL		bIsNT;

	returnValue = sys_GetOSVersion(TRUE, params);

	//sprintf(debugStr,"GetOSVersion return value: %d",returnValue);
	//MessageBox(NULL,debugStr,"Debugging",MB_OK);

	if (returnValue >= OS_NT351) {
		bIsNT = TRUE; // windows and NT have different subkeys to timezone info
	} else {
		bIsNT = FALSE;
	}
	returnValue = -1; //assume all goes well
	standardName_len = PA_GetTextParameter( params, 1, standardName );
  standardName[standardName_len] = '\0';  // Explicitly set the length
	daylightName_len = PA_GetTextParameter( params, 2, daylightName );
  daylightName[daylightName_len] = '\0';
	autoDaylight = PA_GetLongParameter( params, 3);

	if (bIsNT) {
		strcpy ( subKey, "SYSTEM\\ControlSet001\\Control\\TimeZoneInformation");
		strcat (autoDaylightSubkeyName, "Set");
	} else {
		strcpy ( subKey, "SYSTEM\\CurrentControlSet\\Control\\TimeZoneInformation");
	}
	rootKey = HKEY_LOCAL_MACHINE;
	hKey = HKEY_CURRENT_USER; // will get new handle of open key

	errorCode = RegOpenKeyEx ( rootKey, subKey, 0, KEY_READ, &hKey );
	...
	errorCode = RegQueryValueEx ( hKey, "StandardName", NULL, &dwType, sz, &resultSize );
	if (errorCode != ERROR_SUCCESS) {
		strcpy(standardName, "n/a"); // some keyNames may be there & some may not
	} else {
		strcpy(standardName, sz);
	...
	dwType = REG_DWORD;
	dwSize = sizeof(DWORD);
	errorCode = RegQueryValueEx ( hKey, autoDaylightSubkeyName, NULL, &dwType, (PCHAR) &dwAutoDaylight, &dwSize );
	if (errorCode == ERROR_FILE_NOT_FOUND) {
		dwAutoDaylight = 0; // if checkbox checked there will not be an entry in registry
	} else {
		returnValue = returnValue + 3;
	} //(errorCode != ERROR_SUCCESS) 
	
	errorCode = RegQueryValueEx ( hKey, "DaylightName", NULL, &dwDaylightType, szDaylight, &dwDaylightSize );
	if (errorCode != ERROR_SUCCESS) {
		strcpy(daylightName, "n/a"); // some keyNames may be there & some may not
	} else {
		strcpy(daylightName, szDaylight);
		daylightName_len = strlen(daylightName);
		returnValue = returnValue + 20;
	} //(errorCode != ERROR_SUCCESS) 

	errorCode = RegCloseKey( hKey );
	autoDaylight = abs(dwAutoDaylight - 1); // invert the result for 4D representation
	PA_SetTextParameter( params, 1, standardName, standardName_len );
	PA_SetTextParameter( params, 2, daylightName, daylightName_len );
...
void gui_GetDisplayFontDPI( PA_PluginParameters params)
{
	...
	long		systemPath_len = strlen(systemPath);
	returnValue = 0;

	OSVersion = sys_GetOSVersion(TRUE, params);

	systemPath_len = GetWindowsDirectory(systemPath, systemPath_len);
	if (systemPath[systemPath_len] != '\\') {
		strcat(systemPath, "\\");
	}
	strcat(systemPath, "system.ini");
	switch (OSVersion)
	{
		...
		case OS_NT4 :
			...
			strcpy ( subKey, "Software\\Microsoft\\Windows NT\\CurrentVersion\\Fonts");
			...
			errorCode = RegQueryValueEx ( hKey, "Small Fonts (VGA res)", NULL, &dwSZType, strLine, &dwSZSize );
			if (errorCode != ERROR_SUCCESS) {	
				errorCode = RegQueryValueEx ( hKey, "Small Fonts (8514a res)", NULL, &dwSZType, strLine, &dwSZSize );
				if (errorCode != ERROR_SUCCESS) {	
					returnValue = 0;
					PA_ReturnLong( params, returnValue );
					return;
		...
		default :
		if ((cfFile = fopen( systemPath, "r")) != NULL) {
			while (!feof( cfFile )) {
				fscanf( cfFile, "%s", strLine );
				strPosition = strstr( strLine, "aspect");
				if ( strPosition != NULL ) {
					dpiStrPtr = strtok(strLine, ","); // get aspect=100,
					dpiStrPtr = strtok(NULL, ","); // get dpi pointer
					dpi = atol(dpiStrPtr);
					break;
				} // (strPosition != NULL)
			} //while
			fclose(cfFile);
		} //((cfFile = fopen( systemPath, "r")) != NULL) 
...
void sys_SetDefPrinter( PA_PluginParameters params )
{
  ...
  printerName[printerName_len] = '\0';  // Explicitly set the length
	...
	// MJG 3/29/04 Changed the timeout period to 100 from 1000 ms.
	SendMessageTimeout(HWND_BROADCAST, WM_WININICHANGE, 0L,
	    			   (LPARAM)(LPCTSTR)"windows", SMTO_NORMAL, 100, NULL);
...
long sys_GetOSVersion(BOOL bInternalCall, PA_PluginParameters params)
{
  ...
  } else if ((osvinfo.dwMajorVersion == 6) & (osvinfo.dwMinorVersion == 0) & (osvinfo.dwPlatformId == VER_PLATFORM_WIN32_NT)) {
...
long enumPrintersUsingINI( PA_Variable *printerArray )
{
	...
	char					defPrinter[80], szSectionBuf[1000], szDevice[80], sep[2];
