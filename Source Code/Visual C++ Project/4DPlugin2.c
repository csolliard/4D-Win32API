...
void gui_LoadBackground( PA_PluginParameters params, BOOL DeInit )
{
	...
	case 0 :  // assume tiled if no param
		...
		break;
	case BM_SCALE :
		returnValue = sys_GetOSVersion(TRUE, params); // 95 doesn't support dragFullWindows
		// unless Windows Plus installed
		if (returnValue == OS_WIN95) {
			if (!SystemParametersInfo(SPI_GETWINDOWSEXTENSION, 1, NULL, 0)) { // if Plus installed returns true
				break;
			}
		}
...
// ------------------------------------------------
// 
//  FUNCTION: gui_GetWindowMetrics( PA_PluginParameters params)
//
...
void gui_GetWindowMetrics( PA_PluginParameters params )
{
