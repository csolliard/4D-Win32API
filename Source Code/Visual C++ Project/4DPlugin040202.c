...
//	DATE:			07/15/02 dcc
//
//  MODIFICATIONS: 01/16/03 added intercept for WM_GETMINMAXINFO ti limit
	...
	if (toolBarRestrictions.toolBarOnDeck == 1) { // 01/17/03
		...
		switch (uMsg)
		{
			...
			case WM_SETFOCUS : // msg received by window getting focus
				if (currentChildMaxed == (HWND)wParam) { 
...
void sys_DirectoryExists( PA_PluginParameters params ){
	...
	if( NULL != directory ){
		if((  ':' == directory[1] ) && ( 3 == length )) {
			...
			directory[ length-1 ] = '\0'; // remove trailing \ 
		}
	}
