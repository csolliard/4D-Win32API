...
{
	OSVERSIONINFO	osvInfo;            // Structure to hold Windows version.
  ...
  DOCINFO di;                       // Structure to hold "document" information
	...
	char data[MAXBUF] = "";           // String to hold the data param ($2)
	...
	// The following code checks the version of Windows so that it can call the proper Default printer code
	// If osvinfo.dwMajorVersion is >= 5, then we have 2K, XP or 2003 and can use Get/Set Def Printer
	// Otherwise we have to use the profile string.
    ...
    // Get name of current Default Printer
		...
		printerName[printerName_len] = '\0'; // Restrict this since 255 is maxlen for printerName
    GetProfileString("windows", "device", ",,,", origDefault, printerName_len);
		ret = WriteProfileString("windows", "device", printerName);
    ...
    if(osvInfo.dwMajorVersion >= 5)
      ...
      // Set the new Default Printer back to the original.
  		ret = WriteProfileString("windows", "device", origDefault);
